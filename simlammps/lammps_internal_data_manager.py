...
class LammpsInternalDataManager(object):
    """  Class managing Lammps data information using INTERNAL
        communications (i.e. through of the library interface
        to the simulation tool where possible).

        Class manages data existing in Lammps in lammps and
        allows user to query and change them.
